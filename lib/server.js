...
LHServer.prototype.use = function(path, options, fn) {
    if (typeof path === 'function') {
        fn = path;
    } else if (isRouteHandler(path)) {
        fn = path.getHandler();
    }

    if (typeof options === 'function') {
        fn = options;
    } else if (isRouteHandler(options)) {
        fn = options.getHandler();
    }

    if (!fn) {
        return;
    } else if (isRouteHandler(fn)) {
        fn = fn.getHandler();
    }
...
LHServer.prototype.addRoute = function (method, path, handler) {
    this.router.addRoute(method, path, handler);
}

LHServer.prototype.get = function (path, handler) {
    this.router.get(path, handler);
}

LHServer.prototype.post = function (path, handler) {
    this.router.post(path, handler);
}

LHServer.prototype.put = function (path, handler) {
    this.router.put(path, handler);
}

LHServer.prototype.delete = function (path, handler) {
...
function isRouteHandler(object) {
    return object && typeof object === 'object' && typeof object.getHandler === 'function';
}
